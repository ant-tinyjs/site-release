...
var Layer = function() {
  ...
  var lamp = this.createLamp(0, 0, width, height);
  ...
  var self = this;
  var action = new Tiny.Action(2000, Tiny.point(width - 100, height -100));

  action.yoyo = true;
  action.repeatTimes = Infinity;
  action.setEasing(Tiny.TWEEN.Easing.Quadratic.InOut);
  action.onUpdate = function (tween, object) {
    var lamp = self.createLamp(tween.x, tween.y, width, height);

    curtain.mask = lamp;
  };
  action.create()(this).start();
};
